...
# sfc_it = geosphere::areaPolygon(wrld_simpl[wrld_simpl$UN==380, ])
...
# it_sfc = data.frame(rural = 0, 
#                     urban = (siem[country=="Italy", length(unique(city))] * 100 * 1e6) / sfc_it)

al_sfc = data.frame(rural = sum(sapply(sqpols, geosphere::areaPolygon)) / (sfc_fr + sfc_de + sfc_nl + sfc_be + sfc_uk + sfc_ch), # + sfc_it),
